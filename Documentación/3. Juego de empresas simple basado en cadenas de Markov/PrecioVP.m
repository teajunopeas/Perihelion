...
fprintf('Escriba el nuevo precio de venta al p�blico de la empresa %s \n', Empresa(i))
...
while PVPmod(i,j)==PVP(i,j) && salida==1;
    ...
    nomodificacion= Unodos(); %Se comprueba si es 1 � 2.
...
if salida==1 %Si se han modificado los precios, variar�n las ventas
    ...
    for m=1:4; %Bucle en empresas
        if i==m %Si se trata de la empresa que modific� sus precios
        ...
        else %Si no se trata de la empresa que modific� los precios
            CM(m,i)=CM(m,i)-ICM/3; %Las ventas de las otras empresas var�an en sentido contrario equitativamente  
            if CM(m,i)<0 %si es negativo lo hago cero y lo reparto entre los positivos
               CM(m,i)=0;
            end
        end
